...
function readFeedMessage(tag, message, pbf) {
  if (tag === 1) message.header = pbf.readMessage(readFeedHeader, {});
    ...
    if (message.entities.length >= 100_000)
      throw new Error('Transit entity limit exceeded');
...
export function decodeVehiclePositions(bytes) {
  ...
  if (message.header?.incrementality === 1)
    throw new Error('Differential GTFS-RT feeds are unsupported');
  const byId = new Map();
