...
test('untrusted vehicle identity and text fields have conservative bounds', () => {
  const base = {
    id: 'ok',
    vehicle: {
      position: { latitude: 42, longitude: -71 },
      vehicle: { id: 'x'.repeat(257), label: 'z'.repeat(5000) },
    },
  };
  assert.equal(normalizeVehicleEntity(base)?.id, 'ok');
  assert.equal(normalizeVehicleEntity(base)?.label, null);
  assert.equal(normalizeVehicleEntity({ ...base, id: 'y'.repeat(257) }), null);
});

test('differential feeds are rejected rather than deleting valid full snapshot vehicles', () => {
  const pbf = new PbfWriter();
  embed(pbf, 1, (header) => {
    header.writeStringField(1, '2.0');
    header.writeVarintField(2, 1);
  });
  assert.throws(() => decodeVehiclePositions(pbf.finish()), /differential/i);
});
